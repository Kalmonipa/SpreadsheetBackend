...
/// A class that takes positive integers only
class GRPosInteger : Token {
    init(){
...
/// A token representing the column for AbsoluteCell class
class GRColumnLabel : Token {
    /// an uppercase letter representing the column
    init(){
        // parse upper case letters
        super.init(regExpPattern: "[A-Z]+")
    }
    // Takes the string value and returns it as the column
    override func parse(input: String) -> String? {
        if let returnValue = super.parse(input: input) {
            return returnValue
        }
        return nil
    }
...
/// A token representing the row num for AbsoluteCell class
class GRRowNumber : Token {
    /// a positive integer representing the row num
    init(){
    ...
    override func parse(input: String) -> String? {
        let returnValue = super.parse(input: input)
        if let strValue = self.stringValue {
            //Changes the string value to an integer so we can know the row num
            self.calculatedValue = Int(strValue)
        }
        return returnValue
    }
}
...
/// A token representing a string without the quote marks. Simplifies the input for expression reading
class GRStringNoQuote : Token {
    init() {
        super.init(regExpPattern: "[0-9a-zA-Z]")
    }
    override func parse(input: String) -> String? {
        let returnValue = super.parse(input: input)
        return returnValue
